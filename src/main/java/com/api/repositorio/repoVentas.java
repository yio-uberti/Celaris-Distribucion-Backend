...
public interface repoVentas extends CrudRepository<Ventas, Integer>{
	...
	List<Ventas> findAllByTenantIdAndEstadoAndFechaEntrega(
		    Long tenantId, String estado, LocalDate fecha);
	
	//Top de los 5 productos del dia
	@Query(value = "SELECT p.nombre_producto, SUM(dv.cantidad) as totalVendida " +
	        "FROM producto p " +
	        "JOIN detalleventa dv ON p.id_prod = dv.id_producto " +
